...
<div class="container mx-auto p-4">
    ...
    <a href="<?= isset($_GET['from']) && $_GET['from'] === 'bestel-status' ? '/bestel-status' : '/items/items-index' ?>"
        class="bg-[#52EBEB] text-white px-2 py-1 rounded mb-2 inline-block hover:bg-[#3FBFBF]">Terug</a>
    ...
    <div class="flex flex-col md:flex-row items-center md:items-start">
        <img src="/images/<?= htmlspecialchars($item['afbeelding']) ?>" alt="<?= htmlspecialchars($item['naam']) ?>"
            class="w-full md:w-1/2 h-64 object-contain mb-4 md:mb-0 md:mr-4 rounded">
        <div class="md:w-1/2">
            <p class="text-lg text-gray-700"><strong>Naam:</strong> <?= htmlspecialchars($item['naam']) ?></p>
            <p class="text-lg text-gray-700"><strong>Beschrijving:</strong>
                <?= htmlspecialchars($item['beschrijving']) ?></p>
            <p class="text-lg text-gray-700"><strong>Kleur:</strong> <?= htmlspecialchars($item['kleur'] ?? '-') ?></p>
            <p class="text-lg text-gray-700"><strong>Geslacht:</strong>
                <?= htmlspecialchars($item['geslacht'] ?? '-') ?></p>
            <p class="text-lg text-gray-700"><strong>Prijs:</strong>
                &euro;<?= number_format((float)$item['prijs'], 2, ',', '.') ?></p>
            ...
            <form id="addToCartForm" action="/cart/add" method="post" class="mt-2">
                ...
                <button type="submit" class="bg-[#52EBEB] text-white px-4 py-2 rounded hover:bg-[#3FBFBF]">
